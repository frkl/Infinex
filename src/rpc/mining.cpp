...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock)
{
    ...
    bool fAccepted = ProcessNewBlock(Params(), pblock, true, NULL, NULL);
...
UniValue setgenerate(const UniValue& params, bool fHelp)
{
	...
	GenerateInfinex(fGenerate, nGenProcLimit, Params(), *g_connman);
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Infinex Core is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Infinex Core is downloading blocks...");

    if(!IsExceedMinimumChainWork())
		throw JSONRPCError(RPC_VERIFY_ERROR, "Tip chain work lesser than minimum chain work");

	if(!IsBestBlockHashFound())
		throw JSONRPCError(RPC_VERIFY_ERROR, "Best block hash not found in chain");

    // when enforcement is on we need information about a masternode payee or otherwise our block is going to be orphaned by the network
    CScript payee;
    if (sporkManager.IsSporkActive(SPORK_MASTERNODE_PAYMENT_ENFORCEMENT)
        && !masternodeSync.IsWinnersListSynced()
        && !mnpayments.GetBlockPayee(chainActive.Height() + 1, payee))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Infinex Core is downloading masternode winners...");
    ...
    CAmount masternodeReward = GetMasternodeReward(pindexPrev->nHeight+1);
    if(masternodeReward>0) {
        ...
        result.push_back(Pair("masternode_payments_enforced", sporkManager.IsSporkActive(SPORK_MASTERNODE_PAYMENT_ENFORCEMENT)));
    }
    return result;
}
